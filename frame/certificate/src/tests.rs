...
fn last_event() -> CertEvent {
	System::events()
		.into_iter()
		.map(|r| r.event)
		.filter_map(|e| if let Event::Certificate(inner) = e { Some(inner) } else { None })
		.last()
	...
	F: FnOnce(<Test as frame_system::Config>::AccountId, CertId, IssuedId),
{
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Certificate::issue(
			...
			(*ORG_CERT_REF).clone(),
			b"Dave Grohl".to_vec(),
			Some(vec![Property::new(b"satu".to_vec(), b"1".to_vec())]),
			None,
			None
		));
		...
		assert_eq!(issued_cert.props, Some(vec![Property::new(b"satu".to_vec(), b"1".to_vec())]));
...
fn create_cert_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Certificate::create(Origin::signed(Bob.into()), CertDetail::new(last_org_id)));
		match last_event() {
...
fn issue_cert_with_account_handler_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(IssuedCertOwner::<Test>::get(&org_id, &account), Some(vec![issued_id]));
	});
}
...
fn only_org_admin_can_revoke() {
	with_org_cert_issued(|org_id, _cert_id, issued_id| {
		...
		assert_err_ignore_postinfo!(
			Certificate::revoke(Origin::signed(Charlie.into()), org_id, issued_id.clone(), true),
			Error::<Test>::PermissionDenied
		);
