...
fn finalize_works() {
	TestExternalitiesBuilder::default()
		...
		.execute_with(|| {
			...
			pallet_migration::Pallet::<MockRuntime>::finalize(Origin::root()).unwrap();

			assert_noop!(
				pallet_migration::Pallet::<MockRuntime>::finalize(Origin::root()),
				pallet_migration::Error::<MockRuntime>::OnlyFinalizeOngoing,
			);

			assert_noop!(
				pallet_migration::Pallet::<MockRuntime>::migrate_balances_issuance(
					Origin::root(),
					0u32.into()
				),
				pallet_migration::Error::<MockRuntime>::MigrationAlreadyCompleted,
			);

			assert_noop!(
				pallet_migration::Pallet::<MockRuntime>::migrate_system_account(
					Origin::root(),
					Vec::new(),
				),
				pallet_migration::Error::<MockRuntime>::MigrationAlreadyCompleted,
			...
			assert_noop!(
				pallet_migration::Pallet::<MockRuntime>::migrate_proxy_proxies(
					Origin::root(),
					Vec::new()
				),
				pallet_migration::Error::<MockRuntime>::MigrationAlreadyCompleted,
			...
			assert_noop!(
				pallet_migration::Pallet::<MockRuntime>::migrate_vesting_vesting(
					Origin::root(),
					Vec::new()
				),
				pallet_migration::Error::<MockRuntime>::MigrationAlreadyCompleted,
	...
	TestExternalitiesBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				...
				pallet_migration::Error::<MockRuntime>::TooManyAccounts
			);
		});
}
	...
	TestExternalitiesBuilder::default()
		...
		.execute_with(|| {
			...
			for event in reward_events() {
				...
				// actual id
				let not = pallet_migration::Event::<MockRuntime>::FailedToMigrateVestingFor(
	...
	TestExternalitiesBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				...
				pallet_migration::Error::<MockRuntime>::TooManyVestings
			);
		});
}
	...
	TestExternalitiesBuilder::default()
		...
		.execute_with(|| {
			...
			for event in reward_events() {
				...
				// actual id
				let not =
					pallet_migration::Event::<MockRuntime>::FailedToMigrateProxyDataFor(
						AccountId32::from(bytes_id),
					);
	...
	TestExternalitiesBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				...
				pallet_migration::Error::<MockRuntime>::TooManyProxies
			);
		});
}
