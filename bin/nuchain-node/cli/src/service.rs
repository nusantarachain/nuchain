...
// Copyright (C) 2020-2022 Rantai Nusantara Technology Foundation.
...
use futures::prelude::*;
use nuchain_runtime::RuntimeApi;
...
pub fn create_extrinsic(
	...
	function: impl Into<nuchain_runtime::Call>,
	nonce: Option<u32>,
) -> nuchain_runtime::UncheckedExtrinsic {
	...
	let period = nuchain_runtime::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let tip = 0;
	let extra: nuchain_runtime::SignedExtra = (
		frame_system::CheckNonZeroSender::<nuchain_runtime::Runtime>::new(),
		frame_system::CheckSpecVersion::<nuchain_runtime::Runtime>::new(),
		frame_system::CheckTxVersion::<nuchain_runtime::Runtime>::new(),
		frame_system::CheckGenesis::<nuchain_runtime::Runtime>::new(),
		frame_system::CheckEra::<nuchain_runtime::Runtime>::from(generic::Era::mortal(
			period,
			best_block.saturated_into(),
		)),
		frame_system::CheckNonce::<nuchain_runtime::Runtime>::from(nonce),
		frame_system::CheckWeight::<nuchain_runtime::Runtime>::new(),
		pallet_asset_tx_payment::ChargeAssetTxPayment::<nuchain_runtime::Runtime>::from(
			tip, None,
		),
	);

	let raw_payload = nuchain_runtime::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			(),
			nuchain_runtime::VERSION.spec_version,
			nuchain_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	nuchain_runtime::UncheckedExtrinsic::new_signed(
		function,
		sp_runtime::AccountId32::from(sender.public()).into(),
		nuchain_runtime::Signature::Sr25519(signature),
		extra,
	)
}
